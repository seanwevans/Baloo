...
_start:
    ...
    dec rcx                             ;number of file operands
    mov r15, rcx
    mov rbx, rbp
...
.arg_loop:
    ...
    mov r8, rax                         ;fd
    mov rsi, [rbx]                      ;restore filename pointer for output
...
do_cksum:
    ...
    push rbx
    push r8
    xor r12d, r12d                      ;POSIX cksum CRC starts at 0
    xor r13, r13                        ;total bytes
    mov r14, rsi                        ;preserve filename for output
.read_loop:
    mov rax, SYS_READ
    mov rdi, r8
    mov rsi, buffer
...
.length_loop:
    ...
    jz .print_result
    push rax
    call update_crc_byte                ;process low byte of length
    pop rax
    shr rax, 8
    jmp .length_loop
.print_result:
